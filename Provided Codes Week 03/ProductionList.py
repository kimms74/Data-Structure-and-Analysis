...
class ProductionList():
    def __init__(self, Filename):
        ...
        f = open(Filename)
        temp = f.readlines()                            # readlines(): save each lines of file to elements of list
        f.close()

        dataset = []
        for row in temp:
            dataset.append(row[:-1].split(','))         # string.split(':'): split string with ' ' to elements of list 
        Dataset = np.asarray(dataset[1:]).T             # ':'.join(list): joint list with ' ' to string
    ...
    def showPlanChart(self):
        ...
        plt.figure(1)
        plt.subplot(211)
        Uniq_allModel = list(set(allModel))                             #set(list): removing duplicated values and listing unordered 
        Counting_allModel = [allModel.count(a) for a in Uniq_allModel]  #list.count(a): counting number of a in list
